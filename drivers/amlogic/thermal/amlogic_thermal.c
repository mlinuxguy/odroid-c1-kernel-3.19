			...
			struct thermal_cooling_device *cdev)
{
	...
	if(!strcmp(type,"gpufreq")){
		...
		/* Bind the thermal zone to the cpufreq cooling device */
		for (i = 0; i < pdata->temp_trip_count; i++) {
/* fix bad ptr			if(!gpufreq_dev->get_gpu_freq_level){
				ret = -EINVAL;
				pr_info("invalidate pointer %p\n",gpufreq_dev->get_gpu_freq_level);
				goto out;
			} */
/* fix bad ptrs			pdata->tmp_trip[i].gpu_lower_level=gpufreq_dev->get_gpu_freq_level(pdata->tmp_trip[i].gpu_upper_freq);
