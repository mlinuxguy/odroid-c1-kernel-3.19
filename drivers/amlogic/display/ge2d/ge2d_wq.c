...
static int ge2d_process_work_queue(ge2d_context_t *  wq)
{
	...
	do {
		...
		while(ge2d_is_busy())
/* fix	interruptible_sleep_on_timeout(&ge2d_manager.event.cmd_complete, 1); */
		prepare_to_wait(&ge2d_manager.event.cmd_complete, &wait, TASK_INTERRUPTIBLE);
		timeout = schedule_timeout(timeout);
		finish_wait(&ge2d_manager.event.cmd_complete, &wait);
		//if block mode (cmd)
		if(block_mode)
		{
...
int ge2d_wq_add_work(ge2d_context_t *wq)
{
	...
	ge2d_queue_item_t  *pitem ;
 	...
 	if(work_queue_no_space(wq))
 	{
		...
		{
/* fix			interruptible_sleep_on_timeout(&ge2d_manager.event.cmd_complete, 3); */
			prepare_to_wait(&ge2d_manager.event.cmd_complete, &wait, TASK_INTERRUPTIBLE);
			timeout = schedule_timeout(timeout);
			finish_wait(&ge2d_manager.event.cmd_complete, &wait);
		}
